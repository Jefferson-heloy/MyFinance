        ...
        public void Insert()
        {
            ...
            else
            {
                sql = $"UPDATE TRANSACAO SET DATA='{DateTime.Parse(Data).ToString("yyyy/MM/dd")}', " +
                    $"DESCRICAO='{Descricao}', Tipo='{Tipo}' , VALOR='{Valor}', CONTA_ID='{Conta_Id}', " +
                    $"PLANO_CONTAS_ID= '{Plano_Contas_Id}'  WHERE USUARIO_ID= '{IdUsuarioLogado()}' AND ID='{Id}'";
            }
            DAL objDAL = new DAL();
            DataTable dt = objDAL.RetDataTable(sql);
        }
    ...
    public class Dashboard
    {
        ...
        public List<Dashboard> RetornarDadosGraficoPie()
        {
            ...
            string sql = "SELECT sum(t.valor) as total, p.Descricao from transacao as t inner join plano_contas as p " +
                $"on t.Plano_Contas_Id = p.Id where t.Tipo='Despesa' and t.usuario_id={IdUsuarioLogado()} group by p.Descricao";
            DAL objDAL = new DAL();
